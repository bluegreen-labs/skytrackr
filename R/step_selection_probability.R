# This code is originally written by Raphael
# Nussbaumer within the context of the {GeoPressureR} package
# both bird_create() and the step selection probabilities
# are copied here as not to create deep dependencies
# on the whole framework. Some small adjustments were
# made in error handling as well as the automatic
# adjustment of body frontal area calculation depending
# on the species order.


#' Compute probability of a bird speed
#'
#' Converts a speed (airspeed or ground speed) to a
#' probability based on the movement model.
#'
...
speed2prob <- function(speed, movement) {
  ...
  if(!is.numeric(speed)){cli::cli_abort("speed is not numeric")}
  if(!(all(speed >= 0))){cli::cli_abort("negative speeds found")}

  # We use a normalization so that methods are comparable to each other.
  # The normalization is computed as the sum of probability with a
  # 1km/h unit grid
  ...
  if (movement$method == "gamma") {
    ...
    norm <- sum(
      movement$power2prob(
        speed2power(norm_speed * 1000 / 60 / 60, movement$bird)
        )
      )

    prob <- movement$power2prob(
      speed2power(
        speed * 1000 / 60 / 60,
        movement$bird)
      ) / norm
...
speed2power <- function(as, bird) {
  if(!is.numeric(as)){cli::cli_abort("airspeed is not numeric")}
  if(!(all(as >= 0))){cli::cli_abort("negative airspeeds found")}
  if(!inherits(bird, "bird")){cli::cli_abort("bird not of class 'bird'")}
  ...
  # Induced power factor k=1.1-1.2 for aircraft and helicopter,
  # ad k=1.04 in Spedding (1987a) (p. 45).
  k <- 1.2
  ...
  # Induce power (eq 16 of Box 3.1) Pind is due to the active
  # acceleration of mass flow in order to
  # produce a force opposing weight and drag
  p_ind <- 2 * k * (bird$mass * g)^2 / (as * pi * bird$wing_span^2 * rho)

  # Parasitic power (eq 3 of Box 3.2) (also called Body Power)
